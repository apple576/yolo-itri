...
class WTConvNeXtBlock(BaseModule):
    ...
    Args:
        ...
        dw_conv_cfg (dict): Config of depthwise WTConv convolution. Defaults to ``dict(kernel_size=5, stride=1,
            bias=True, wt_levels=2)``.
        norm_cfg (dict): The config dict for norm layers. Defaults to ``dict(type='LN2d', eps=1e-6)``.
        act_cfg (dict): The config dict for activation between pointwise
            convolution. Defaults to ``dict(type='GELU')``.
        mlp_ratio (float): The expansion ratio in both pointwise convolution. Defaults to 4.
    ...
    def __init__(
        ...
        if act_cfg is None:
            act_cfg = {"type": "GELU"}
        if norm_cfg is None:
            norm_cfg = {"type": "LN2d", "eps": 1e-06}
        if dw_conv_cfg is None:
            dw_conv_cfg = {"kernel_size": 5, "stride": 1, "bias": True, "wt_levels": 2}
        super().__init__()
        self.with_cp = with_cp

        self.depthwise_conv = _WTConv2d(in_channels, in_channels, **dw_conv_cfg)
...
class WTConvNeXt(BaseBackbone):
    ...
    A PyTorch implementation of `A ConvNet for the 2020s <https://arxiv.org/abs/2201.03545>`_ and `ConvNeXt V2:
    Co-designing and Scaling ConvNets with Masked Autoencoders <http://arxiv.org/abs/2301.00808>`_

    Modified from the `official repo <https://github.com/facebookresearch/ConvNeXt/blob/main/models/convnext.py>`_ and
    `timm <https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/convnext.py>`_.

    To use WTConvNeXt v2, please set ``use_grn=True`` and ``layer_scale_init_value=0.``.

    Args:
        arch (str | dict): The model's architecture. If string, it should be one of architecture in
            ``ConvNeXt.arch_settings``. And if dict, it
        ...
        frozen_stages (int): Stages to be frozen (all param fixed). Defaults to 0, which means not freezing any
            parameters.
        gap_before_final_norm (bool): Whether to globally average the feature map before the final norm layer. In the
            official repo, it's only used in classification task. Defaults to True.
        with_cp (bool): Use checkpoint or not. Using checkpoint will save some memory while slowing down the training
    ...
    arch_settings = {
        ...
        # },
        "tiny": {"depths": [3, 3, 9, 3], "channels": [96, 192, 384, 768], "wt_levels": [5, 4, 3, 2]},
        "small": {"depths": [3, 3, 27, 3], "channels": [96, 192, 384, 768], "wt_levels": [5, 4, 3, 2]},
        "base": {"depths": [3, 3, 27, 3], "channels": [128, 256, 512, 1024], "wt_levels": [5, 4, 3, 2]},
        # 'large': {
        #     'depths': [3, 3, 27, 3],
    ...
    def __init__(
        ...
        if init_cfg is None:
            init_cfg = [
                ...
                {"type": "Constant", "layer": ["LayerNorm"], "val": 1.0, "bias": 0.0},
            ]
        ...
        if isinstance(arch, str):
            ...
            assert "depths" in arch and "channels" in arch, (
                f'The arch dict must have "depths" and "channels", but got {list(arch.keys())}.'
            )
            ...
            stage = Sequential(
                *[
                    WTConvNeXtBlock(
                        ...
                        dw_conv_cfg={"kernel_size": 5, "stride": 1, "bias": True, "wt_levels": wt_levels},
