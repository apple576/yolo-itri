...
from wtconvnext import wtconvnext_base, wtconvnext_small, wtconvnext_tiny
...
def create_wtconvnext_cuda(size="tiny"):
    ...
    if size == "tiny":
        return wtconvnext_tiny(
            pretrained=False, wtconv_class=WTConv2dCUDA, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE
        )
    elif size == "small":
        return wtconvnext_small(
            pretrained=False, wtconv_class=WTConv2dCUDA, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE
        )
    else:
        return wtconvnext_base(
            pretrained=False, wtconv_class=WTConv2dCUDA, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE
        )
...
def create_wtconvnext_triton(size="tiny"):
    ...
    if size == "tiny":
        return wtconvnext_tiny(
            pretrained=False, wtconv_class=WTConv2dTriton, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE
        )
    elif size == "small":
        return wtconvnext_small(
            pretrained=False, wtconv_class=WTConv2dTriton, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE
...
def main():
    ...
    models = [
        ...
        (
            ...
            lambda: timm.create_model(
                "convnext_tiny", pretrained=False, kernel_sizes=CONVNEXT_KERNEL_SIZE, conv_mlp=USE_CONV_MLP
            ),
            True,
        ),
    ]
    ...
    if BENCHMARK_REGULAR:
        models.append(
            (
                "WTConvNeXt-T",
                lambda: wtconvnext_tiny(pretrained=False, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE),
                False,
            )
        )
    ...
    models.extend(
        [
            (
                "ConvNeXt-S",
                lambda: timm.create_model(
                    "convnext_small", pretrained=False, kernel_sizes=CONVNEXT_KERNEL_SIZE, conv_mlp=USE_CONV_MLP
                ),
                True,
            ),
        ]
    ...
    if BENCHMARK_REGULAR:
        models.append(
            (
                "WTConvNeXt-S",
                lambda: wtconvnext_small(pretrained=False, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE),
                False,
            )
        )
    ...
    models.extend(
        [
            (
                "ConvNeXt-B",
                lambda: timm.create_model(
                    "convnext_base", pretrained=False, kernel_sizes=CONVNEXT_KERNEL_SIZE, conv_mlp=USE_CONV_MLP
                ),
                True,
            ),
        ]
    ...
    if BENCHMARK_REGULAR:
        models.append(
            (
                "WTConvNeXt-B",
                lambda: wtconvnext_base(pretrained=False, conv_mlp=USE_CONV_MLP, kernel_sizes=WTCONVNEXT_KERNEL_SIZE),
                False,
            )
        )
    ...
    for name, model_factory, is_baseline in models:
        ...
        if (
            ("ConvNeXt-T" in name and "WT" not in name)
            or ("ConvNeXt-S" in name and "WT" not in name)
            or ("ConvNeXt-B" in name and "WT" not in name)
        ):
            pass
        ...
        is_last_in_group = (not BENCHMARK_TRITON and "CUDA" in name) or (BENCHMARK_TRITON and "Triton" in name)
        if is_last_in_group:
            print("-" * 42)
