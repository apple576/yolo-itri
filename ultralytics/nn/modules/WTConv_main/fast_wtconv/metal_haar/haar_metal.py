...
class InverseHaarTripleTransform(Function):
    ...
    def forward(ctx, level1, level2, level3, output_size):
        ...
        _get_module().haar2d_triple_backward(level1.contiguous(), level2.contiguous(), level3.contiguous(), output)
    ...
    def backward(ctx, grad_output):
        ...
        _get_module().haar2d_triple_forward(grad_output.contiguous(), grad_level1, grad_level2, grad_level3)
...
class InverseHaarQuadTransform(Function):
    ...
    def backward(ctx, grad_output):
        ...
        _get_module().haar2d_quad_forward(grad_output.contiguous(), grad_level1, grad_level2, grad_level3, grad_level4)
...
class ScaledDepthwiseConvFunction(Function):
    ...
    Fuses scale into weight before conv: y = conv(x, scale * weight) This uses the backend (MPS/cuDNN) for both forward
    and backward, giving ~1.17x training speedup over separate conv + scale_mul.
    """
...
def scaled_depthwise_conv(
    ...
    This is the RECOMMENDED function for training. It fuses scale into weights before the convolution, using the backend
    (MPS/cuDNN) for both forward and backward passes. Provides ~1.17x training speedup over separate conv + scale_mul.
