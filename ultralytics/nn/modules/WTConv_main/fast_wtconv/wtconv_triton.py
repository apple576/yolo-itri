...
class WTConv2d(nn.Module):
    ...
    def __init__(
        ...
        self.base_weight = nn.Parameter(torch.Tensor(in_channels, 1, kernel_size, kernel_size))
        ...
        for _ in range(wt_levels):
            ...
            self.wt_weights.append(nn.Parameter(torch.randn(in_channels * 4, 1, kernel_size, kernel_size) * 0.02))
            self.wt_scales.append(nn.Parameter(torch.ones(1, in_channels * 4, 1, 1) * 0.1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        base_out = scaled_depthwise_conv(
            x, self.base_weight, self.base_scale, self.kernel_size // 2, bias=self.base_bias
        )
        ...
        for i, (weight, scale) in enumerate(zip(self.wt_weights, self.wt_scales)):
            ...
            _Hi, _Wi = current_input.shape[2], current_input.shape[3]
            ...
            if need_ll:
                ...
                out, ll_raw = fused_haar_conv_scale(current_input, weight, scale, self.kernel_size, return_ll=True)
                current_input = ll_raw
            else:
                out = fused_haar_conv_scale(current_input, weight, scale, self.kernel_size, return_ll=False)
        ...
        if self.wt_levels == 1:
            ...
            output = ihaar2d_double_fused(level_outputs[0], level_outputs[1], base_out, (H, W))

        elif self.wt_levels == 3:
            output = ihaar2d_triple_fused(level_outputs[0], level_outputs[1], level_outputs[2], base_out, (H, W))

        elif self.wt_levels == 4:
            output = ihaar2d_quad_fused(
                level_outputs[0], level_outputs[1], level_outputs[2], level_outputs[3], base_out, (H, W)
            )
