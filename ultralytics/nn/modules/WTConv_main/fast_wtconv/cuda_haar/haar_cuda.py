...
def ihaar2d_cascade(levels, output_size):
    """Fused multi-level inverse Haar cascade. Takes list of coefficient tensors and reconstructs to full resolution in
    one kernel.
...
def ihaar2d(x: torch.Tensor, output_size: tuple[int, int] | None = None) -> torch.Tensor:
...
class InverseHaarTripleTransform(Function):
    ...
    def forward(ctx, level1, level2, level3, output_size):
        ...
        _get_module().ihaar2d_triple_cascade(level1.contiguous(), level2.contiguous(), level3.contiguous(), output)
    ...
    def backward(ctx, grad_output):
        ...
        _get_module().haar2d_triple_cascade(grad_output.contiguous(), grad_level1, grad_level2, grad_level3)
...
class InverseHaarQuadTransform(Function):
    ...
    def forward(ctx, level1, level2, level3, level4, output_size):
        ...
        _get_module().ihaar2d_quad_cascade(
            level1.contiguous(), level2.contiguous(), level3.contiguous(), level4.contiguous(), output
        )
    ...
    def backward(ctx, grad_output):
        ...
        _get_module().haar2d_quad_cascade(grad_output.contiguous(), grad_level1, grad_level2, grad_level3, grad_level4)
...
class InverseHaarQuintTransform(Function):
    ...
    def backward(ctx, grad_output):
        ...
        _get_module().haar2d_quint_cascade(
            grad_output.contiguous(), grad_level1, grad_level2, grad_level3, grad_level4, grad_level5
        )
...
class ScaledDepthwiseConvFunction(Function):
    ...
    Fuses scale into weight before conv: y = conv(x, scale * weight) This uses cuDNN for both forward and backward,
    giving ~1.17x training speedup.
    """

    @staticmethod
    def forward(
        ctx, input: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor, padding: int, groups: int
    ) -> torch.Tensor:
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        input, weight, _scale, fused_weight = ctx.saved_tensors
        padding = ctx.padding
        groups = ctx.groups

        # grad_input uses fused_weight (cuDNN backward)
        grad_input = torch.nn.grad.conv2d_input(input.shape, fused_weight, grad_output, padding=padding, groups=groups)
...
def scaled_depthwise_conv(
    input: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor, padding: int = 1
) -> torch.Tensor:
    """Scaled depthwise convolution: output = scale * depthwise_conv(input, weight).

    This is the RECOMMENDED function for training. It fuses scale into weights before the convolution, using cuDNN for
    both forward and backward passes. Provides ~1.17x training speedup over separate conv + scale_mul.
