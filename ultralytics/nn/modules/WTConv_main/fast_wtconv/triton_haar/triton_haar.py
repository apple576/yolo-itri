...
def compute_scaled_weight(weight: torch.Tensor, scale: torch.Tensor, kernel_size: int = 3) -> torch.Tensor:
    """Precompute scaled weights for the optimized kernel.

    Instead of a (C, 4, K2, K2) effective kernel, we compute (C, 4, K, K) weights that operate on Haar partial sums
    directly.
...
def _fused_haar_conv_scale_kernel(
    ...
    Key insight: For each 2x2 input block at (2*kh, 2*kw), the Haar transform produces partial sums:
...
def _fused_haar_conv_scale_backward_kernel(
    ...
    Iterates over output positions (H2 x W2), computing gradients for a 2x2 input block. Each thread writes to a UNIQUE
    2x2 block, so no atomics needed.
    """
    pid = tl.program_id(0)
    offs = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offs < N
    ...
    for kh in tl.static_range(K):
        for kw in tl.static_range(K):
            ...
            grad_ll = tl.load(grad_out_base + 0 * stride_gos + idx_base, mask=valid, other=0.0)
            grad_lh = tl.load(grad_out_base + 1 * stride_gos + idx_base, mask=valid, other=0.0)
            grad_hl = tl.load(grad_out_base + 2 * stride_gos + idx_base, mask=valid, other=0.0)
            grad_hh = tl.load(grad_out_base + 3 * stride_gos + idx_base, mask=valid, other=0.0)
...
def _compute_grad_weight_scale(
    ...
    grad_fused_weight = torch.nn.grad.conv2d_weight(haar_coeffs, weight.shape, grad_output, padding=padding, groups=C4)
...
class FusedHaarConvScaleFunction(torch.autograd.Function):
    ...
    Forward: Applies Haar transform, depthwise conv with scaled weights. Backward: Computes gradients for input, weight,
    and scale.
    """
    ...
    def backward(ctx, grad_output, grad_ll=None):
        ...
        if needs_weight_grad or needs_scale_grad:
            ...
            grad_weight, grad_scale = _compute_grad_weight_scale(haar_coeffs, grad_output_flat, weight, scale, K)
...
def fused_haar_conv_scale(
    ...
    Uses an optimized kernel that exploits Haar symmetry for 4x reduced kernel memory traffic compared to the naive
    effective kernel approach.
    ...
    Args:
        ...
        scale: (1, C*4, 1, 1) or (C*4,) scales
        kernel_size: K
        return_ll: If True, returns (coeffs, ll_raw) where ll_raw is (B, C, H/2, W/2) containing the raw LL subband
            (AvgPool) for the next level.
