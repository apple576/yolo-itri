...
class ScaledDepthwiseConvFunction(torch.autograd.Function):
    ...
    Fuses scale into weight before conv: y = conv(x, scale * weight) + bias This uses cuDNN for both forward and
    backward, giving ~1.17x training speedup.
    """
...
def scaled_depthwise_conv(
    input: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor, padding: int = 1, bias: torch.Tensor = None
) -> torch.Tensor:
