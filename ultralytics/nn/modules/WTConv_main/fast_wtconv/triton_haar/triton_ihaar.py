...
def _ihaar_step(ll, lh, hl, hh):
    """Inverse Haar step: Reconstruct 2x2 block from 4 subbands. Uses FMA-friendly computation pattern.
    """
...
def _ihaar_cascade_kernel(
    ...
    W5: tl.constexpr,
    # Strides
    stride_l1_plane: tl.constexpr,
    stride_l1_row: tl.constexpr,
    stride_l2_plane: tl.constexpr,
    stride_l2_row: tl.constexpr,
    stride_l3_plane: tl.constexpr,
    stride_l3_row: tl.constexpr,
    stride_l4_plane: tl.constexpr,
    stride_l4_row: tl.constexpr,
    stride_l5_plane: tl.constexpr,
    stride_l5_row: tl.constexpr,
    stride_out_plane: tl.constexpr,
    stride_out_row: tl.constexpr,
    # Add tensor strides (used when HAS_ADD=True)
    stride_add_plane: tl.constexpr,
    stride_add_row: tl.constexpr,
    # Constants
    ...
    """IHaar cascade kernel: 4 outputs per thread for better compute/load ratio. Grid is over B*C*H1*W1 (Level 1
    coefficients).
    """
    pid = tl.program_id(0)
...
def run_haar_cascade(x, num_levels):
    """Run forward Haar cascade (Backward of Inverse Haar). Computes coefficients for each level by iteratively calling
    single-level kernel.
...
class InverseHaarCascadeFn(torch.autograd.Function):
    ...
    Forward: (Coeffs1, Coeffs2...) -> Image (Triton) Backward: Grad_Image -> (Grad_Coeffs1, Grad_Coeffs2...) (Triton -
    Forward Cascade)
    """
...
class InverseHaarCascadeFusedFn(torch.autograd.Function):
    ...
    Forward: (Coeffs1, Coeffs2..., add_tensor) -> Image + add_tensor (Triton, fused) Backward: Grad_Output ->
    (Grad_Coeffs1, ..., Grad_add_tensor)
